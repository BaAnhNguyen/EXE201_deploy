...
async function main() {
  ...
  // Seed TRIAL package
  console.log('Seeding TRIAL_14_DAYS package...');
  const trialPackage = await prisma.subscription.upsert({
    where: { package_code: 'TRIAL_14_DAYS' },
    update: { price: 0, billing_cycle: 'TRIAL', is_active: true },
    create: {
      package_code: 'TRIAL_14_DAYS',
  ...
  console.log(`Upserted Package: ${trialPackage.package_code}`);

  // 2. Fetch all subscriptions we care about
  const packages = ['BASIC_MONTHLY', 'BASIC_YEARLY', 'ULTIMATE_MONTHLY', 'ULTIMATE_YEARLY', 'TRIAL_14_DAYS'];
