...
export class SubscriptionPurchaseController {
  ...
  getPendingTrialRequests() {
    return this.purchaseService.getPendingTrialRequests();
  }

  @Post('confirm-trial/:orderCode')
  confirmTrialPayment(@Param('orderCode') orderCode: string) {
    return this.purchaseService.confirmTrialPayment(orderCode);
  }
  // Thêm vào controller
  ...
  // API to seed trial package
  @Post('seed-trial')
  async seedTrial() {
    return this.purchaseService.seedTrialPackage();
  }
